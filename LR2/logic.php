<?php
    ...
    if (isset($_GET["cost1"]) && $_GET["cost1"] != "") 
        $stmt->bindValue('cost1GET', $_GET['cost1'], PDO::PARAM_INT);
    if (isset($_GET["cost2"]) && $_GET["cost2"] != "") 
        $stmt->bindValue('cost2GET', $_GET['cost2'], PDO::PARAM_INT);
    if (isset($_GET["type"]) && $_GET["type"] != "") 
        $stmt->bindValue('typeGET', $_GET['type'], PDO::PARAM_STR);
    if (isset($_GET["description"]) && $_GET["description"] != "") 
        $stmt->bindValue('descGET', '%' . $_GET['description'] . '%', PDO::PARAM_STR);
    if (isset($_GET["name"]) && $_GET["name"] != "") 
        $stmt->bindValue('nameGET', '%' . $_GET['name'] . '%', PDO::PARAM_STR);
    $stmt->execute();
    $data = array();
